...
reference_model_outputs = 'D:/Model Runs/REF_MODEL/Sim_99999_v2'  # directory with the model outputs (also where processed outputs will be saved)
...
# time period:
start_year = 1979   # timeseries will start Oct 1 of start_year
end_year = 2022     # timeseries will end Sep 30 of end_year
...
catchment_wide_avg_timeseries = process_model_timeseries(reference_model_outputs, Glacier_ID, sim, years, Sfc, False)

# Save the catchment-wide average timeseries for each variable as text files:
save_timeseries(list(catchment_wide_avg_timeseries),years,debrisfree_outputs,varnames,'REF_MODEL','krh')

# Calculate the catchment-wide average standard deviation from the ensemble mean:
catchment_wide_avg_timeseries_stddev = process_model_timeseries(reference_model_outputs, Glacier_ID, sim, years, Sfc, True)

# Save the standard deviation timeseries as text files:
save_timeseries(list(catchment_wide_avg_timeseries_stddev),years,debrisfree_outputs,varnames,'REF_MODEL','krh_std')
...
glacier_wide_avg_timeseries = process_model_timeseries(reference_model_outputs, Glacier_ID, sim, years, KW_grid, False)

# Save the glacier-wide average timeseries for each variable as text files:
save_timeseries(list(glacier_wide_avg_timeseries),years,reference_model_outputs,varnames,'REF_MODEL','kw')

# Calculate the glacier-wide average standard deviation from the ensemble mean:
glacier_wide_avg_timeseries_stddev = process_model_timeseries(reference_model_outputs, Glacier_ID, sim, years, KW_grid, True)

# Save the standard deviation timeseries as text files:
save_timeseries(list(glacier_wide_avg_timeseries_stddev),years,reference_model_outputs,varnames,'REF_MODEL','kw_std')

# =============================================================================
# Repeat, but averaged over the entire glacierized area
# =============================================================================

# Calculate the daily glacier-wide average of each variable:
# Note: Here, Sfc is replaced with KW_grid, which defines the new area to be averaged over.
allglacier_wide_avg_timeseries = process_model_timeseries(reference_model_outputs, Glacier_ID, sim, years, glacierized_area_grid, False)

# Save the glacier-wide average timeseries for each variable as text files:
save_timeseries(list(allglacier_wide_avg_timeseries),years,reference_model_outputs,varnames,'REF_MODEL','allgl')

# Calculate the glacier-wide average standard deviation from the ensemble mean:
allglacier_wide_avg_timeseries_stddev = process_model_timeseries(reference_model_outputs, Glacier_ID, sim, years, glacierized_area_grid, True)

# Save the standard deviation timeseries as text files:
save_timeseries(list(allglacier_wide_avg_timeseries_stddev),years,reference_model_outputs,varnames,'REF_MODEL','allgl_std')
...
distributed_fields = process_model_distributed_fields(reference_model_outputs, Glacier_ID, sim, years, False)
save_distributed_fields(list(distributed_fields),varnames,years,debrisfree_outputs,'REF_MODEL',False)

# compute and save annual distributed standard deviation fields:
distributed_fields_std = process_model_distributed_fields(reference_model_outputs, Glacier_ID, sim, years, True)
save_distributed_fields(list(distributed_fields_std),varnames,years,debrisfree_outputs,'REF_MODEL',True)
